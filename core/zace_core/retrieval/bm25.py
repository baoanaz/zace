...
- FTS 返回**原始 bm25 分（越小越相关）并已按它升序**——本模块把它转成 1-based 排名；
...
def bm25_query_text(query: str) -> str:
    ...
    理由：jieba 会把 `` ` `` 与 ``:`` 切成独立 token，而正文里没有这些 token，FTS5 的隐式
    AND（TASK-001 ``fts_search``）会让整条查询失配——显式符号链查询（``TokenService::refresh``）
    尤其明显。其它通道与渲染一律用原文，本归一化只作用于 BM25 的 MATCH 串。
    """
