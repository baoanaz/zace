...
8. **MANIFEST.sha256 语义**（2026-09-10 明确）：它是设计文档迁入 zace 仓库时的**快照记录**（用于核对“研究目录快照”与仓库版本的对应关系），不是持续校验门禁。设计文档的后续变更由编排者执行并经 git 历史追踪；变更后不强制重算 manifest（如需校对快照，用 `git log -- docs/design/` 对照）。实现期的口径与漂移记录统一写在 `docs/plan/contracts.md` §3。
