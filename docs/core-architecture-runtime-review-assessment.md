...
| 原评审条目 | 判断 | 理由 |
|---|---|---|
| **P1-2** 删除文件可靠清理向量 | **立即做** | 已承认缺陷、改动小（`Store` 加批量接口）、不触碰检索语义 → **跑分零影响** |
| **P2-1** query cache 提到 Engine 生命周期 + 引入 model 身份 | **立即做** | 事实核实：service 路径 `Engine.open()` 未注入 cache，60s 复用**确实不存在**；单次查询结果不变 → **跑分零影响** |
| **P2-5** `files.generated` 恒为 0 | **立即做** | `store.py:351` 硬编码 `0` 属实；落库后 generated 文件降权 → **MRR 可能上升，须前后对照** |
| **P1-1** 索引非原子提交 | **做简版** | 认问题，**不认 generation 双目录**：`~/.zace/bench/` 下已有 7 个持久索引（langchain 179 MB），改目录布局等于全部重建。先落它自己给的备选方案 `index-state.json` |
| **P1-5** 查询可能读到中间态 | **做简版** | 认同风险，但**不引入 ProjectActor**：2 vCPU 上常驻资源池是过度设计。先让 search trace 暴露该状态 |
| **P1-3** 向量超时线程不可取消 | **降为 P2** | 事实属实，但原评审低估了兜底：`api.py` 已有 `httpx` `connect=10s / total=60s`，通道超时 5s，堆积窗口有上限，非"无限积累"。先补 deadline 透传 |
| **P1-4** 并行召回 + 四分支轻路由 | **认同低优先级** | 原评审 §3 已自行标注可接受；且其依据的跑分口径已漂移，须先重测 |
| **P2-2** `reasons` 字符串当控制协议 | **推迟** | 判断正确（`rerank.py:266` / `engine.py:781` 属实），但 sidecar 改造动 `Candidate` 消费面，属重构 |
| **P2-3** 候选对象可变污染 | **推迟** | 现象已被 `param-sweep` §2 独立发现并留档，无新增信息 |
| **P2-4** 图扩展 N+1 SQL | **推迟** | 当前池约 120，未成为实测瓶颈 |
| **P2-6** 拆 assembly + 边际效用装填 | **不采纳** | 与 `R29/R30` 冻结纪律及"4 类参数敏感度为 0"的实测冲突；且原报告自述"必须离线 replay 对比，不直接替换"——**当前没有可用的 replay 口径**（§1.2） |
| **P2-7** Answer 归属 core/service | **转契约讨论** | 判断正确：`interfaces.py:80` 与 `Module/06 §5` 说 `llm/` 属 core，实际 `core/zace_core/llm/` **不存在**，全在 `service/zace_service/answer.py`。但这要走 `docs/plan/orchestration.md §4` 契约变更流程，**不自行改实现** |
| **§6-3** 删除 `chain_priority` 越界启发式 | **暂不删** | 设计文档已标注"建议删除"，但它现在是 `engine.py:954` 排序键的首位成分，**且为 09-17 两次修复留下**。删除前必须 replay A/B（同 §1.2 阻塞） |
...
3. **做原评审 P1-2 / P2-1 / P2-5 三项止血**——改动小、跑分影响可界定。
4. **其余条目挂到 TASK-093 之后**，作为真实数据驱动的评估输入，而不是直接排期。
5. **P2-7 开契约讨论**（Answer 归属），走 §4 流程，不在本轮改实现。
